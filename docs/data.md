...
- [`archive_content`](/docs/configuration/#archive-content) followed by
  `agentsview sync --full` applies a whole-archive storage policy. Unlike
  category filtering, the rebuild also projects orphaned and trashed sessions
  onto the policy, so dropped tool payloads or transcript text leave the
  archive entirely.
