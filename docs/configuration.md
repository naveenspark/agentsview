...
| `"transcripts"` | Message text, thinking text, titles, and tool call metadata (names, categories, result lengths, statuses, subagent links). Tool inputs and tool results, usually most of a large archive, are dropped. |
...
The policy applies when rows are written, so changing it does not rewrite rows
already in the archive. To apply a narrower policy to an existing archive, set
the key and run `agentsview sync --full`. The rebuild re-parses sessions whose
source files still exist and copies archived sessions whose sources are gone,
projecting both onto the policy. Back up the archive first: dropped content
cannot be restored without the original source files.
...
- `"transcripts"` keeps session search, transcript viewing, and tool analytics
  ...
  additional effect under this policy.
- `"usage"` supports usage reports such as `agentsview usage daily`. Session
  ...
  metrics require a fuller archive.
...
| `archive_content`                   | How much of each session the archive stores: `"full"` (default), `"transcripts"`, or `"usage"` — see [Archive content](#archive-content)                                                                                                                  |
